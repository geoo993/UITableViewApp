...
class ProductLine {
    ...
    class var allProducts : Int {
        ...
        get {
            return (ProductLine.films().products.count + ProductLine.tvSeries().products.count +
                ProductLine.videoGames().products.count + ProductLine.music().products.count + ProductLine.books().products.count + ProductLine.countries().products.count)
    ...
    private class func countries () -> ProductLine
    {
        ...
        products.append(Product(title: "No Countries", description: "Sorry Can't Compile ",imageName: "", year: 0, rating: ProductRating.Unrated, genres: [Genres.None]))
        return ProductLine(productType: "Countries", includedProducts: products)
    }
